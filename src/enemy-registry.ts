...
export type EnemyTargetingMode = "standard" | "flag" | "player" | "harvester" | "priority" | "archer" | "acidslinger" | "rammer";
export type EnemyAttackMode = "melee" | "arrow" | "acid" | "ram" | "boss";
export type EnemyDeathMode = "none" | "split" | "burst";
export type EnemyTargetKind = "player" | "flag" | StructureKind;
...
export interface EnemyDefinition {
  ...
  capabilities: {
    ...
    canUseTunnels: boolean;
    fireAura?: boolean;
    radiationAura?: boolean;
    meleeRetaliation?: {
      kind: "burn";
  ...
  targeting: {
    ...
    innerRadius?: number;
    lockSeconds: number;
    priorities?: readonly EnemyTargetKind[];
  };
  movement: {
...
type EnemyDefinitionInput = Omit<EnemyDefinition, "capabilities" | "ram"> & {
  capabilities?: Partial<EnemyDefinition["capabilities"]>;
  ram?: Omit<NonNullable<EnemyDefinition["ram"]>, "cooldownSeconds"> & {
    cooldownSeconds?: number;
  };
};
...
export function enemyTargetPriorities(definition: EnemyDefinition): readonly EnemyTargetKind[] {
  if (definition.targeting.priorities?.length) return definition.targeting.priorities;
  switch (definition.targeting.mode) {
    case "flag": return ["flag"];
    case "player": return ["player", "flag"];
    case "harvester": return ["harvester", "flag"];
    case "archer":
    case "acidslinger": return ["turret", "player", "flag"];
    case "rammer": return ["wall", "flag"];
    case "priority": return ["flag"];
    case "standard": return ["flag", "player", "turret", "harvester"];
  }
}
...
export function enemyRenderDimensions(
  definition: EnemyDefinition,
  campaignTierId: CampaignTierId,
): { width: number; height: number } | null {
  const render = definition.render;
  if (!render) return null;
  const normalizedSpecial = definition.rosterEligible
    && definition.campaignTierIds?.includes(campaignTierId)
    && campaignTierId !== "forest"
    && campaignTierId !== "snowy";
  const height = render.height * (normalizedSpecial ? 1.2 : 1);
  return { width: height * render.aspectRatio, height };
}
...
export function selectEnemyRoster(seed: string, campaignTierId: CampaignTierId = "forest"): EnemyRoster {
  ...
  const fixedCampaignSpecials = campaignTierId === "forest"
    ? []
    : campaignTier(campaignTierId).specialEnemies;
  for (const kind of fixedCampaignSpecials) {
